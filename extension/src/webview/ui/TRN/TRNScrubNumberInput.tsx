...
const SCRUB_ACTIVATION_THRESHOLD_PX = 4;
...
export function TRNScrubNumberInput(props: TRNScrubNumberInputProps) {
  ...
  const displayDecimals = useMemo(
    () => computeTrnScrubDisplayDecimals(min, max, step, fractionDigits),
    [fractionDigits, max, min, step],
  );
  ...
  const onPointerDown = (e: ReactPointerEvent<HTMLInputElement>) => {
    ...
    const onMove = (evt: PointerEvent) => {
      ...
      if (!s.scrubbing) {
        if (dx * dx + dy * dy < SCRUB_ACTIVATION_THRESHOLD_PX * SCRUB_ACTIVATION_THRESHOLD_PX) {
          return;
        }
        s.scrubbing = true;
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "ArrowUp") {
      e.preventDefault();
      const next = clamp(value + step * mult, min, max);
      onChange(next);
  ...
  const onWheel = (e: ReactWheelEvent<HTMLInputElement>) => {
  ...
  return (
    <input
      ...
      type="text"
      inputMode="decimal"
      title={fullPrecisionTitle}
      aria-label={ariaLabel}
      aria-readonly={locked ? true : undefined}
      className={twMerge(
        "nodrag nopan nowheel [-moz-appearance:textfield] min-w-0 flex-1 appearance-none bg-transparent text-right font-mono text-[11px] tabular-nums text-zinc-100 outline-none",
