...
import type { Dps368ControlCommonProps } from "../types";
import { SENSOR_CFG_UI } from "../../../constants/sensorConfigUiLabels";

export function DPS368OperationCard(
  props: Dps368ControlCommonProps & {
    collapsed: boolean;
    ...
    >
      <div className="mb-2 flex items-center justify-between rounded border border-zinc-700/80 bg-transparent px-2 py-1 text-sm">
        ...
        <TRNToggleSwitch
          ...
          ariaLabel="Toggle DPS368 sensor enabled"
        />
      </div>

      <div className={`${controlsDisabled ? "pointer-events-none opacity-50" : ""} rounded border border-zinc-700/80 bg-transparent px-2 py-1 text-sm`}>
        ...
        <div role="radiogroup" aria-label="DPS368 telemetry mode" className="flex w-full flex-row flex-wrap justify-between gap-2">
          <TRNButton className="min-w-14 flex-1 border-zinc-700/80" size="compact" selected={publishMode === 0} onClick={() => onPublishModeChange(0)}>
            Periodic
          </TRNButton>
          <TRNButton className="min-w-14 flex-1 border-zinc-700/80" size="compact" selected={publishMode === 1} onClick={() => onPublishModeChange(1)}>
            Change
          </TRNButton>
          <TRNButton className="min-w-14 flex-1 border-zinc-700/80" size="compact" selected={publishMode === 2} onClick={() => onPublishModeChange(2)}>
            Hybrid
          </TRNButton>
        </div>
      </div>
