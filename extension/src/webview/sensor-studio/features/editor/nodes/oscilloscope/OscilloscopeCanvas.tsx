...
import type { OscilloscopeChannelStyle, OscilloscopeConfig } from "./oscilloscope-config";
...
function lineDash(style: OscilloscopeChannelStyle["lineStyle"], lineWidth: number): number[] {
  switch (style) {
    case "dashed":
...
function drawMarkers(
  ...
  xs: number[],
  ys: number[],
  style: OscilloscopeChannelStyle["marker"],
  color: string,
  lineWidth: number,
): void {
...
/** Stable legend strip height so plot height does not jump when the first sample arrives. */
function legendBandPx(
  showLegend: boolean,
  channelOrder: readonly string[],
  channels: OscilloscopeConfig["channels"],
  hCss: number,
): number {
  if (!showLegend) {
    return 0;
...
export function OscilloscopeCanvas(props: OscilloscopeCanvasProps) {
  ...
  schedulePaintRef.current = () => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      /** Layout box of wrapper — matches CSS `inset-0` sizing (avoid px mismatch vs getBoundingClientRect). */
      ...
      const sampleCap = cfg.sampleCount;
      const series: { id: string; pts: number[]; sty: OscilloscopeChannelStyle }[] = [];
      for (const id of order) {
        ...
        const raw = h[id] ?? [];
        const slice = raw.slice(-sampleCap);
