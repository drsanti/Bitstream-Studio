...
import { InspectorUtilityDetailsContent } from "./InspectorUtilityDetailsContent";
import {
  catalogAboutLead,
  resolveSensorInspectorAboutForNode,
} from "./sensor-inspector-about-content";
import { resolveUtilityInspectorDetailsContent } from "./utility-inspector-details-content";
...
export function NodeInspectorDetailsTab(props: NodeInspectorDetailsTabProps) {
  ...
  const utilityDetails = useMemo(
    () => resolveUtilityInspectorDetailsContent(selectedNode.data.nodeId),
  ...
  const visibleSectionIds = useMemo((): DetailsInspectorSectionId[] => {
    ...
    if (description.length > 0 || sensorAbout != null || utilityDetails != null) {
      ids.push("specs");
    }
    ...
    return ids;
  }, [description, sensorAbout, utilityDetails, portRows.length]);
      ...
      return (
        <TRNInteractiveCard title="Specifications" {...cardProps}>
          {sensorAbout != null ? (
            ...
            <InspectorUtilityDetailsContent
              content={utilityDetails}
              catalogLead={catalogAboutLead(description)}
            />
          ) : (
            <InspectorDetailsMarkdown markdown={description} />
          )}
        </TRNInteractiveCard>
      );
    }
