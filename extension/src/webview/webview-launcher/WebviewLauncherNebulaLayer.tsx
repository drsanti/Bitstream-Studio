...
function drawStar(
  ...
  const t = timeMs * 0.001;
  const twinkle = 0.35 + 0.65 * (0.5 + 0.5 * Math.sin(t * star.twinkleSpeed * 1000 + star.twinklePhase));
  const x =
    star.x + Math.cos(star.driftAngle + t * star.driftSpeed) * star.depth * 18;
  const y =
    star.y + Math.sin(star.driftAngle + t * star.driftSpeed * 0.85) * star.depth * 12;
...
/**
 * Deep-space nebula: drifting, rotating gas clouds + star particles (top band fades into shell).
 */
export function WebviewLauncherNebulaLayer() {
