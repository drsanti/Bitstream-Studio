...
## BS2 Wi-Fi on UART (host ↔ CM55)

CM55 exposes IPC Wi-Fi on the **same BS2 wire as sensors** (`"BS "` prefix, CRC16, CRLF). Uses `REQ` / `RES` / `EVT_STATUS` — **not** legacy `0xAA55` channel `0x06`.

| Document | Content |
|----------|---------|
| **`TESAIoT_Library/CM55/modules/bitstream/docs/WIFI_BS2_ASYNC_PROTOCOL.md`** | Normative async flows, `reqId`, `EVT_STATUS` kinds |
| **`t3d-extension/docs/BITSTREAM_BS_FRAMED_PROTOCOL_SPEC.md`** | §8.3 Wi-Fi `cmdId`, §10 `EVT_STATUS` |
| **`t3d-extension/src/bitstream2/docs/WIFI_BS2.md`** | Host implementation guide |
| **`protocol/include/bitstream_bs_wifi.h`** | Firmware constants |

**IPC → BS2 mapping (device → host):**

| IPC (CM33 → CM55) | BS2 to host |
|-------------------|-------------|
| `IPC_EVT_WIFI_SCAN_RESULT` | `EVT_STATUS` `WIFI_SCAN_ROW` (`kind=0x02`) — **one frame per AP** |
| `IPC_EVT_WIFI_SCAN_COMPLETE` | `EVT_STATUS` `WIFI_SCAN_DONE` (`kind=0x03`) |
| `IPC_EVT_WIFI_STATUS` | `EVT_STATUS` `WIFI_LINK` (`kind=0x01`; `reqId=0` if unsolicited) |
| `IPC_EVT_WIFI_POLICY` | `EVT_STATUS` `WIFI_POLICY` (`kind=0x04`) or `RES` for GET |

**Correlation:** host **`reqId`** in `REQ`/`RES`/`EVT_STATUS` (no separate `corr_id`).

**Implementation status (2026-06-03):** UART baseline shipped — firmware `bitstream_bs_wifi_service.c`, host `bitstream2/domains/wifi/*`, Bitstream Studio Device Wi-Fi UI. Follow-ups: simulator parity, golden fixtures — see `WIFI_BS2_ASYNC_PROTOCOL.md` checklist.
